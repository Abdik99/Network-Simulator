...
class ThreeTenNetwork {
	 ...
	 * @param queue queue
	 * @param hostToDijkstraNode hostToDijkstraNode
	 */
	public static void setupDijkstras(Network graph, Host startNode, PriorityQueue<DijkstraNode> queue, HashTable<Host,DijkstraNode> hostToDijkstraNode) {
		...
		//You'll need to "wrap" nodes from your graph into DijkstraNodes and add
		...
		for (Host v:graph.getVertices())
		{
			if (!v.equals(startNode))
			{
				...
				DijkstraNode temp = new DijkstraNode(v);

				queue.add(temp);
				hostToDijkstraNode.add(v,temp);
			}
	...
	public static void setupTables(Network graph, Host startNode, HashTable<Host,DijkstraNode> hostToDijkstraNode) {
		...
		for (Host v:graph.getVertices())
		{
			...
			DijkstraNode temp = hostToDijkstraNode.get(v);
			...
			if (!v.equals(startNode))
			{
				...
				if (temp.parent!=null)
				{
					...
					while (!temp.parent.host.equals(startNode) && temp.parent!=null)
					{
				...
				if (temp.host!=null && temp.parent!=null && temp.parent.host!=null && temp.parent.host.equals(startNode) && startNode!=null)
				{
					startNode.getRoutingTable().add(v, temp.host);
